    ...
    <div className="pt-page-stack">
      <div className="page-kicker">
        <div>
          ...
          <h1 className="page-title icon-title"><Webhook size={20} /> Webhook Manager</h1>
          ...
          <form className="panel" onSubmit={save}>
            ...
            <div className="panel-body pt-page-stack">
              <div className="form-grid">
                <label className="field"><span className="f-label">Channel</span><select className="f-input" value={form.channel} onChange={(event) => selectChannel(event.target.value as WebhookChannel)} disabled={saving}><option value="teams">Teams</option><option value="discord">Discord</option><option value="telegram">Telegram</option><option value="brrr">BRRR</option></select></label>
                <label className="field"><span className="f-label">Server configuration alias</span><select className="f-input font-mono" value={form.targetRef} onChange={(event) => setForm((current) => ({ ...current, targetRef: event.target.value }))} disabled={saving}>{TARGET_REFS[form.channel].map((targetRef) => <option key={targetRef} value={targetRef}>{targetRef}</option>)}</select></label>
                <label className="field"><span className="f-label">Enabled</span><select className="f-input" value={String(form.enabled)} onChange={(event) => setForm((current) => ({ ...current, enabled: event.target.value === 'true' }))} disabled={saving}><option value="true">Enabled</option><option value="false">Disabled</option></select></label>
              </div>
              <div className="action-row">
            ...
            {webhooks.length > 0 ? (
              <div className="tbl-scroll"><table className="mock"><thead><tr><th>Channel</th><th>Configuration</th><th>Status</th><th>Updated</th><th>Actions</th></tr></thead><tbody>
                {webhooks.map((webhook) => <tr key={webhook.id}>
                  ...
                  <td><span className={`pill${webhook.enabled ? ' ok' : ' neutral'}`}>{webhook.enabled ? 'ENABLED' : 'DISABLED'}</span></td>
            ...
            {deliveries.length > 0 ? (
              <div className="tbl-scroll"><table className="mock"><thead><tr><th>Status</th><th>Webhook ID</th><th>Attempts</th><th>Response</th><th>Updated</th><th>Action</th></tr></thead><tbody>
                {deliveries.map((delivery) => <tr key={delivery.id}>
                  <td><span className={`pill${delivery.status === 'delivered' ? ' ok' : delivery.status === 'failed' ? ' neutral' : ''}`}>{delivery.status.toUpperCase()}</span></td>
