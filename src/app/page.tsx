...
const Badge = ({ children, variant = 'default', className = '' }: { children: React.ReactNode, variant?: string, className?: string }) => {
  const variants: Record<string, string> = {
    default: 'bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 border-zinc-300 dark:border-zinc-700',
...
const IconButton = ({ icon: Icon, onClick, className = '', active = false }: { icon: React.ElementType, onClick?: () => void, className?: string, active?: boolean }) => (
  <button 
    ...
    className={`p-1.5 rounded-md spring-transition flex items-center justify-center
      ${active ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 hover:text-zinc-900 dark:text-white hover:bg-zinc-100 dark:hover:bg-zinc-100 dark:bg-zinc-800/50'} 
      ${className}`}
  >
...
const Button = ({ children, variant = 'primary', icon: Icon, className = '', onClick }: { children?: React.ReactNode, variant?: string, icon?: React.ElementType, className?: string, onClick?: () => void }) => {
  ...
  const variants: Record<string, string> = {
    ...
    ghost: "bg-transparent hover:bg-zinc-100 dark:hover:bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 border-transparent",
  };
    ...
    >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-md spring-transition text-sm group
              ${activeRoute === item.id 
                ...
                : 'text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-100 dark:bg-zinc-800/50 hover:text-zinc-800 dark:hover:text-zinc-800 dark:text-zinc-200'}`}
          >
      ...
      <div className="p-2 border-t border-zinc-200 dark:border-zinc-800 mt-auto flex flex-col gap-1">
        <button 
          ...
          className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-100 dark:bg-zinc-800/50 hover:text-zinc-800 dark:hover:text-zinc-800 dark:text-zinc-200 spring-transition text-sm"
        >
        ...
        <button className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-100 dark:bg-zinc-800/50 hover:text-zinc-800 dark:hover:text-zinc-800 dark:text-zinc-200 spring-transition text-sm">
    ...
    <header className="h-12 border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md flex items-center justify-between px-4 shrink-0 sticky top-0 z-10">
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 text-sm">
          <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">Core API</span>
          <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">/</span>
      ...
      <div className="flex items-center gap-3">
        {/* Global Search Mock */}
        <div className="hidden md:flex items-center gap-2 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md px-2 py-1 text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 text-sm w-64 spring-transition hover:border-zinc-300 dark:hover:border-zinc-300 dark:border-zinc-700">
          <Search size={14} />
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">
        ...
        <div className="flex items-center bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md p-0.5 text-xs font-medium">
          ...
          <button className="px-2 py-1 rounded text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  hover:text-zinc-700 dark:text-zinc-300 dark:hover:text-zinc-300  dark:text-zinc-300 spring-transition">EN</button>
        </div>
        
        {/* Logout */}
        <IconButton icon={LogOut} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 hover:text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:bg-rose-500/10" />
      </div>
    </header>
  );
};
            ...
            <div className="w-full flex justify-between relative z-10">
              {steps.map((step, idx) => (
                <div key={step.id} className="flex flex-col items-center gap-2 bg-white dark:bg-zinc-950 px-2">
                  ...
                  <span className={`text-xs font-medium ${step.status === 'current' ? 'text-zinc-800 dark:text-zinc-200' : 'text-zinc-500 dark:text-zinc-400'}`}>
                    {step.label}
                  </span>
                </div>
              ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {isUploaded ? (
              ...
              <div className="bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-xl p-1">
                <div 
                  onClick={() => setIsUploaded(true)}
                  className="border-2 border-dashed border-zinc-300 dark:border-zinc-700/50 hover:border-indigo-500/50 rounded-lg p-10 flex flex-col items-center justify-center text-center spring-transition group cursor-pointer bg-white/50 dark:bg-zinc-950/50"
                >
                  <div className="w-16 h-16 rounded-full bg-zinc-50 dark:bg-zinc-900 flex items-center justify-center mb-4 group-hover:bg-indigo-50 dark:bg-indigo-500/10 group-hover:scale-110 spring-transition">
                    <UploadCloud size={32} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 group-hover:text-indigo-600 dark:text-indigo-400" />
                  </div>
                  <h3 className="text-base font-medium text-zinc-800 dark:text-zinc-200 mb-1">Upload PRD Document</h3>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  max-w-sm mb-4">
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 space-y-6">
              ...
              <div className="relative">
                <label className="flex items-center justify-between text-sm font-medium text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 mb-3">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      {(() => {
                        ...
                        return active ? (
                          <>
                            <active.icon size={16} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 " />
                            <span>{active.id}</span>
                          </>
                        ) : 'Select Platform';
                      })()}
                    </div>
                    <ChevronDown size={16} className={`text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`} />
                  </button>
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm spring-transition
                          ${selectedPlatform === platform.id 
                            ...
                            : 'text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-100 dark:bg-zinc-800 hover:text-zinc-900 dark:hover:text-zinc-900 dark:text-zinc-100'}`}
                      >
                        <platform.icon size={16} className={selectedPlatform === platform.id ? 'text-indigo-600 dark:text-indigo-400' : 'text-zinc-500 dark:text-zinc-400'} />
              ...
              {/* API Service */}
              <div>
                <label className="flex flex-col text-sm font-medium text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 mb-3">
                  <span>API Service Identifier</span>
                  <span className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  font-normal mt-0.5">Optional: Bind to a specific microservice context</span>
                </label>
                <div className="relative">
                  <Globe size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 " />
                  <input 
                    type="text" 
            ...
            {isUploaded && (
              <div className="bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-xl flex flex-col overflow-hidden shadow-sm">
                ...
                <div className="p-6 overflow-y-auto max-h-[600px] text-sm text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 space-y-8 bg-white dark:bg-zinc-950/30">
                  
                  {/* Title & Metadata */}
                  <div>
                    <h2 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight mb-2">PRD-2026-017: Stock Screener</h2>
                    <p className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  text-sm">Product: Growin&apos;</p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-zinc-100 dark:bg-zinc-900/50 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800/50">
                        <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  mb-2 uppercase tracking-wider font-semibold">Document Control</div>
                      ...
                      <div className="bg-zinc-100 dark:bg-zinc-900/50 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800/50">
                        <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  mb-2 uppercase tracking-wider font-semibold">Metadata</div>
                        <div className="space-y-1.5 text-sm">
                  ...
                  <div>
                    <h3 className="text-base font-medium text-zinc-800 dark:text-zinc-200 mb-3 flex items-center gap-2">
                      <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">1.</span> Background & Problem Statement
                    </h3>
                    <p className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 leading-relaxed bg-zinc-50/80 dark:bg-zinc-900/30 p-4 rounded border border-zinc-100 dark:border-zinc-800/30">
                  ...
                  <div>
                    <h3 className="text-base font-medium text-zinc-800 dark:text-zinc-200 mb-3 flex items-center gap-2">
                      <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">2.</span> Objectives
                    </h3>
                    <ul className="list-disc pl-5 text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 space-y-2">
                      <li><strong className="text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Adoption & Engagement:</strong> Increase daily active users utilizing the screener tool by 25%.</li>
                      <li><strong className="text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Usability:</strong> Reduce time-to-discovery for technical setups to under 60 seconds.</li>
                      <li><strong className="text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Data Integrity:</strong> Ensure 99.9% accuracy of fundamental ratios retrieved from the data vendor.</li>
                      <li><strong className="text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Non-advice Integrity:</strong> Strictly maintain neutral positioning; no automated buy/sell recommendations.</li>
                    </ul>
                  </div>
                  ...
                  <div>
                    <h3 className="text-base font-medium text-zinc-800 dark:text-zinc-200 mb-3 flex items-center gap-2">
                      <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">3.</span> Functional Requirements (F1-F21)
                    </h3>
                    <div className="bg-zinc-100 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden">
                      <table className="w-full text-left text-sm">
                        <thead className="bg-zinc-50 dark:bg-zinc-900/80 border-b border-zinc-200 dark:border-zinc-800">
                          <tr>
                            <th className="px-4 py-3 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 font-medium">ID</th>
                            <th className="px-4 py-3 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 font-medium">Feature</th>
                            <th className="px-4 py-3 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 font-medium">Description</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-zinc-800/50 text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F1</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Popular Screener</td><td className="px-4 py-2.5">Pre-built screening templates based on popular metrics.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F2</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Add Filter</td><td className="px-4 py-2.5">Ability to append specific fundamental/technical criteria.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F3</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Technical Preset</td><td className="px-4 py-2.5">Golden cross, RSI oversold, MACD crossover presets.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F4</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Custom Preset</td><td className="px-4 py-2.5">Save user-defined screening criteria as a reusable preset.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F5</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Condition Builder</td><td className="px-4 py-2.5">Logical AND/OR grouping for complex filtering rules.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F6</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Universe</td><td className="px-4 py-2.5">Filter by index (LQ45, IDX30) or market cap tiers.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F7</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Result Table</td><td className="px-4 py-2.5">Sortable data grid with inline sparklines.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F8</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Backtest</td><td className="px-4 py-2.5">Simulate historical performance of custom screening rules.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F9</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Export</td><td className="px-4 py-2.5">Download results as CSV or PDF format.</td></tr>
                          <tr><td className="px-4 py-2.5 font-mono text-xs">F10</td><td className="px-4 py-2.5 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Alerts</td><td className="px-4 py-2.5">Push notifications when new stocks enter/exit conditions.</td></tr>
                          <tr><td className="px-4 py-3 font-mono text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 text-xs italic bg-white/50 dark:bg-zinc-950/50" colSpan={3}>... 11 more features truncated for preview ...</td></tr>
                        </tbody>
                      </table>
                    </div>
                  </div>
                  ...
                  <div>
                    <h3 className="text-base font-medium text-zinc-800 dark:text-zinc-200 mb-3 flex items-center gap-2">
                      <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">4.</span> Architecture & Data Flow
                    </h3>
...
{`[Client App] --(1. Init Rule)--> [API Gateway] --(2. Validate)--> [Screener Service]
                  ...
                  <div>
                    <h3 className="text-base font-medium text-zinc-800 dark:text-zinc-200 mb-3 flex items-center gap-2">
                      <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">5.</span> Feature Gating (Free vs Premium)
                    </h3>
                    <p className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 leading-relaxed bg-zinc-50/80 dark:bg-zinc-900/30 p-4 rounded border border-zinc-100 dark:border-zinc-800/30">
                  ...
                  <div>
                    <h3 className="text-base font-medium text-zinc-800 dark:text-zinc-200 mb-3 flex items-center gap-2">
                      <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">6.</span> Appendices (Included in Parse)
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">
                      <div className="flex items-start gap-2 bg-zinc-50/80 dark:bg-zinc-900/30 p-2.5 rounded border border-zinc-200 dark:border-zinc-800/50"><CheckCircle2 size={16} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 mt-0.5 shrink-0"/> Definitions & Glossary</div>
                      <div className="flex items-start gap-2 bg-zinc-50/80 dark:bg-zinc-900/30 p-2.5 rounded border border-zinc-200 dark:border-zinc-800/50"><CheckCircle2 size={16} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 mt-0.5 shrink-0"/> Non-Functional Requirements</div>
                      <div className="flex items-start gap-2 bg-zinc-50/80 dark:bg-zinc-900/30 p-2.5 rounded border border-zinc-200 dark:border-zinc-800/50"><CheckCircle2 size={16} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 mt-0.5 shrink-0"/> Release Plan</div>
                      <div className="flex items-start gap-2 bg-zinc-50/80 dark:bg-zinc-900/30 p-2.5 rounded border border-zinc-200 dark:border-zinc-800/50"><CheckCircle2 size={16} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 mt-0.5 shrink-0"/> Acceptance Criteria</div>
                      <div className="flex items-start gap-2 bg-zinc-50/80 dark:bg-zinc-900/30 p-2.5 rounded border border-zinc-200 dark:border-zinc-800/50"><CheckCircle2 size={16} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 mt-0.5 shrink-0"/> Constraints & Change Logs</div>
                    </div>
                  </div>
          ...
          <div className="hidden lg:block space-y-4">
            <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-xl p-5">
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 leading-relaxed mb-4">
              ...
              <ul className="space-y-2 text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">
            ...
            <div className="bg-zinc-50/80 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5">
              <h4 className="text-sm font-medium text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 mb-3">Recent Intakes</h4>
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between group cursor-pointer">
                    <div className="flex items-center gap-2 overflow-hidden">
                      <FileText size={14} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  shrink-0" />
                      <span className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 truncate group-hover:text-zinc-800 dark:text-zinc-200 spring-transition">{item.name}</span>
                    </div>
                    <span className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 shrink-0 ml-2">{item.time}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="flex h-full w-full bg-white dark:bg-zinc-950 overflow-hidden">
      ...
      <div className="flex flex-col h-full border-r border-zinc-200 dark:border-zinc-800" style={{ flex: '1 1 61.8%', minWidth: 0 }}>
        ...
        {/* Table Header */}
        <div className="grid grid-cols-12 gap-4 px-4 py-2 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900/50 text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto">
          {MOCK_REQUIREMENTS.map((req) => (
            ...
            >
              <div className="col-span-2 font-mono text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">{req.id}</div>
              ...
              <div className="col-span-2 flex items-center gap-2">
                ...
                <span className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  w-8">{req.coverage}%</span>
              </div>
              ...
              <div className="col-span-2 flex items-center justify-end gap-2">
                ...
                <span className="font-mono text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">{req.aiConf}%</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col h-full bg-zinc-50/80 dark:bg-zinc-900/30" style={{ flex: '0 0 38.2%', minWidth: '320px' }}>
        {selectedReq ? (
          <>
            <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
              ...
              <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-lg p-4">
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 leading-relaxed">
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 mb-4 uppercase tracking-wider">Generated Test Cases</h3>
                ...
                {[1, 2].map((i) => (
                  <div key={i} className="p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/50 hover:border-zinc-300 dark:hover:border-zinc-300 dark:border-zinc-700 spring-transition group">
                    ...
                    <div className="space-y-1 mt-3">
                      <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  font-mono"><span className="text-indigo-600 dark:text-indigo-400 mr-2">GIVEN</span> user is on login page</div>
                      <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  font-mono"><span className="text-amber-600 dark:text-amber-400 mr-2">WHEN</span> Okta SSO callback returns valid JWT</div>
                      <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  font-mono"><span className="text-emerald-600 dark:text-emerald-400 mr-2">THEN</span> session is created and user redirected</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex-1 flex items-center justify-center text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  text-sm">
...
const CoverageAuditPage = ({ setActiveRoute }: { setActiveRoute: (route: string) => void }) => {
  const steps = [
...
const TestCataloguePage = ({ setActiveRoute }: { setActiveRoute: (route: string) => void }) => {
  const steps = [
    { id: 1, label: 'PRD Intake', icon: FileText, status: 'pending' },
    { id: 2, label: 'Coverage Audit', icon: ShieldCheck, status: 'pending' },
...
const QaseIntegrationPage = ({ setActiveRoute }: { setActiveRoute: (route: string) => void }) => {
  const steps = [
    { id: 1, label: 'PRD Intake', icon: FileText, status: 'pending' },
    { id: 2, label: 'Coverage Audit', icon: ShieldCheck, status: 'pending' },
    { id: 3, label: 'Test Catalogue', icon: Library, status: 'pending' },
    ...
    <div className="flex h-full w-full bg-white dark:bg-zinc-950 overflow-hidden">
      ...
      <div className="flex flex-col h-full border-r border-zinc-200 dark:border-zinc-800" style={{ flex: '1 1 61.8%', minWidth: 0 }}>
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center justify-center relative bg-white/50 dark:bg-zinc-950/50">
          ...
          <div className="w-16 h-16 rounded-full bg-zinc-100 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center mb-4 relative z-10 shadow-lg">
            <MessageSquare size={24} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400" />
          </div>
          <h3 className="text-base font-medium text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 mb-1 relative z-10">No messages yet</h3>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  relative z-10">Send a prompt to test the model.</p>
        </div>
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shrink-0">
          ...
          <div className="flex justify-between items-center mt-2 px-1">
            <span className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 font-mono flex items-center gap-1">
      ...
      <div className="flex flex-col h-full bg-white dark:bg-zinc-950" style={{ flex: '0 0 38.2%', minWidth: '320px' }}>
        ...
        <div className="h-12 border-b border-zinc-200 dark:border-zinc-800 flex items-center px-5 bg-zinc-50/80 dark:bg-zinc-900/30 shrink-0">
          <SlidersHorizontal size={14} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 mr-2" />
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-8 bg-zinc-50 dark:bg-zinc-900/10">
          ...
          {/* Model Selection */}
          <div className="space-y-3">
            <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 uppercase tracking-wider">Model</label>
            <button className="w-full flex items-center justify-between bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-zinc-800 dark:text-zinc-200 hover:border-zinc-300 dark:hover:border-zinc-300 dark:border-zinc-700 spring-transition">
              ...
              <ChevronDown size={14} className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  shrink-0" />
            </button>
          </div>

          {/* Parameters */}
          <div className="space-y-6">
            <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 uppercase tracking-wider">Parameters</label>
            
            {/* Temperature */}
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Temperature</span>
                <span className="text-xs font-mono text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  bg-zinc-50 dark:bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-200 dark:border-zinc-800">{temperature.toFixed(2)}</span>
              </div>
              <input 
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Max Tokens</span>
                <span className="text-xs font-mono text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  bg-zinc-50 dark:bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-200 dark:border-zinc-800">{maxTokens}</span>
              </div>
              <input 
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Top P</span>
                <span className="text-xs font-mono text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  bg-zinc-50 dark:bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-200 dark:border-zinc-800">{topP.toFixed(2)}</span>
              </div>
              <input 
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">Top K</span>
                <span className="text-xs font-mono text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  bg-zinc-50 dark:bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-200 dark:border-zinc-800">{topK}</span>
              </div>
              <input 
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                <Activity size={12} className="text-indigo-600 dark:text-indigo-400" /> Performance
              </label>
              <span className="flex items-center gap-1.5 text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-zinc-100 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
                <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  mb-1 font-medium">Prefill (t/s)</div>
                <div className="text-lg font-mono text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">0.0</div>
              </div>
              <div className="bg-zinc-100 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
                <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  mb-1 font-medium">Token Gen (t/s)</div>
                <div className="text-lg font-mono text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">0.0</div>
              </div>
              <div className="bg-zinc-100 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
                <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  mb-1 font-medium">Thinking (s)</div>
                <div className="text-lg font-mono text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">—</div>
              </div>
              <div className="bg-zinc-100 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
                <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  mb-1 font-medium">Duration (s)</div>
                <div className="text-lg font-mono text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">—</div>
              </div>
            </div>
          </div>
    ...
    <div className="h-full w-full bg-white dark:bg-zinc-950 overflow-y-auto">
      <div className="max-w-7xl mx-auto w-full p-8 flex flex-col gap-6 pb-20">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">Manage integrations, API keys, and generation policies.</p>
        </div>
          ...
          <div className="p-6 space-y-5 bg-transparent dark:bg-zinc-900/20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-2">
                <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Base URL (OpenAI-compatible)</label>
                <div className="relative">
                  <Globe size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400" />
              ...
              <div className="space-y-2">
                <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">API Token <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 font-normal">(Optional)</span></label>
                <div className="relative">
                  <Key size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400" />
              ...
              <div className="space-y-2 md:col-span-2">
                <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Model</label>
                <div className="relative">
                  <Server size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400" />
          ...
          <div className="overflow-x-auto bg-transparent dark:bg-zinc-900/20">
            <table className="w-full text-left text-sm">
              <thead className="bg-white/50 dark:bg-zinc-950/50 border-b border-zinc-200 dark:border-zinc-800">
                <tr>
                  <th className="px-5 py-3 text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 uppercase tracking-wider">Resource</th>
                  <th className="px-5 py-3 text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 uppercase tracking-wider">Endpoint</th>
                  <th className="px-5 py-3 text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 uppercase tracking-wider">Status</th>
                  <th className="px-5 py-3 text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 uppercase tracking-wider">Count</th>
                  <th className="px-5 py-3 text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 uppercase tracking-wider text-right">Last Checked</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800/50 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">
                <tr>
                  <td className="px-5 py-3 font-medium">Projects</td>
                  <td className="px-5 py-3 font-mono text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">/project</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 text-right">—</td>
                </tr>
                <tr>
                  <td className="px-5 py-3 font-medium">Suites</td>
                  <td className="px-5 py-3 font-mono text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">/suite/&#123;projectCode&#125;</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 text-right">—</td>
                </tr>
                <tr>
                  ...
                  <td className="px-5 py-3 font-mono text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">/case/&#123;projectCode&#125;</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 text-right">—</td>
                </tr>
                <tr>
                  <td className="px-5 py-3 font-medium flex items-center gap-2">Runs <span className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 font-normal">(Optional)</span></td>
                  <td className="px-5 py-3 font-mono text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">/run/&#123;projectCode&#125;</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 text-right">—</td>
                </tr>
                <tr>
                  <td className="px-5 py-3 font-medium flex items-center gap-2">Defects <span className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 font-normal">(Optional)</span></td>
                  <td className="px-5 py-3 font-mono text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 ">/defect/&#123;projectCode&#125;</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400">—</td>
                  <td className="px-5 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 text-right">—</td>
                </tr>
        ...
        <div className="bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden flex flex-col">
          <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-100 dark:bg-zinc-900/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  mt-0.5">API Base URL can be overridden server-side via QASE_BASE_URL. API Token is saved locally in your browser.</p>
              </div>
            </div>
          ...
          <div className="p-6 space-y-5 bg-transparent dark:bg-zinc-900/20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-2">
                <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">API Base URL</label>
              ...
              <div className="space-y-2">
                <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">API Token</label>
              ...
              <div className="space-y-2 md:col-span-2">
                <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Project</label>
                <div className="relative">
                  ...
                  <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 pointer-events-none" />
                </div>
              </div>
            </div>
          ...
          <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-100 dark:bg-zinc-900/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  mt-0.5">Standards injected as RAG Context during AI generation.</p>
              </div>
            </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
      {[
        ...
        <div key={i} className="bg-zinc-100 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 flex flex-col justify-between spring-transition hover:border-zinc-300 dark:hover:border-zinc-300 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-50 dark:bg-zinc-900">
          <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 text-sm font-medium">{stat.label}</span>
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-[400px]">
      <div className="flex-1 bg-zinc-50/80 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 flex flex-col" style={{ flex: '1 1 61.8%' }}>
        <div className="flex items-center justify-between mb-4">
          <span className="text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 text-sm font-medium">Test Coverage Trend</span>
          <Button variant="ghost" className="text-xs py-1 h-auto">View Report</Button>
        </div>
        <div className="flex-1 flex items-end justify-between border border-zinc-200 dark:border-zinc-800/50 rounded-lg text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 text-sm bg-white/50 dark:bg-zinc-950/50 p-6 pt-10 relative">
          {/* Y-axis grid lines */}
          <div className="absolute inset-0 p-6 pt-10 pb-12 flex flex-col justify-between pointer-events-none">
            {[100, 75, 50, 25, 0].map(val => (
              <div key={val} className="w-full flex items-center border-b border-zinc-100 dark:border-zinc-800/30 h-0 relative">
                <span className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 absolute -left-2 -translate-x-full bg-white dark:bg-zinc-950 pr-2">{val}%</span>
              </div>
            ))}
          </div>
          ...
          <div className="w-full h-full flex items-end justify-between pl-8 z-10 relative gap-2">
            {[
              ...
              <div key={i} className="flex flex-col items-center gap-3 w-full h-full justify-end">
                ...
                <span className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  font-medium uppercase tracking-wider">{d.month}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col bg-zinc-50/80 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5" style={{ flex: '0 0 38.2%' }}>
        <span className="text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 text-sm font-medium mb-4">Recent AI Activity</span>
           ...
           {[
             ...
             <div key={i} className="flex items-start gap-3">
               ...
               <div className="flex-1 text-sm leading-tight">
                 <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">{event.action} </span>
                 <span className="text-zinc-800 dark:text-zinc-200 font-medium">{event.target}</span>
                 <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-600 dark:text-zinc-400 mt-1">{event.time}</div>
               </div>
             </div>
           ))}
        </div>
...
export default function App() {
  ...
  const renderContent = () => {
    switch (activeRoute) {
      ...
      case 'test-catalogue':
        return <TestCataloguePage setActiveRoute={setActiveRoute} />;
      case 'qase-integration':
        return <QaseIntegrationPage setActiveRoute={setActiveRoute} />;
      case 'ai-playground':
    ...
    <>
      ...
      <div className={`flex h-screen w-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans overflow-hidden selection:bg-indigo-500/30 ${isDarkMode ? 'dark' : ''}`}>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-white dark:bg-zinc-950 relative shadow-[-10px_0_30px_rgba(0,0,0,0.05)] dark:shadow-[-10px_0_30px_rgba(0,0,0,0.5)] z-10">
          ...
          {/* Telemetry Footer */}
          <footer className="h-6 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex items-center px-4 justify-between text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-400  font-mono shrink-0 relative z-20">
