/**
 * Advanced PDF Parser with layout analysis and dictionary recovery
 * Ported from bahan-page-qaautomation for accurate PRD text extraction
 */
      ...
      async function processFile(pdfjs: any) {
        try {
          ...
          for (let i = 1; i <= pdf.numPages; i++) {
            ...
            // Post-process dictionary recovery
            pageText = pageText
              ...
              .replace(/Port\s+f\s+ol\s+i\s+o|Port f ol i o/gi, 'Portfolio')
              .replace(/Anal\s+y\s+s\s+i\s+s|Anal y s i s/gi, 'Analysis')
              .replace(/Perform\s+ance|Perform ance/gi, 'Performance')
              .replace(/Optim\s+i\s+z\s+ation|Optim i z ation/gi, 'Optimization')

            fullText += pageText + '\n\n'
          }
          resolve(fullText)
        } catch (e) {
          reject(new Error(`PDF parsing failed: ${(e as any).message}`))
        }
      }
    })
  },
